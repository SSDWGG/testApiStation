...
const probes = [
  ...
  {
    id: 'self_identity', name: '模型自报身份', weight: 5, minScore: 50, minSeverity: 'pass',
    async run() {
      const claimed = CONFIG.model.toLowerCase();
      const resp = await callAPI([{role:'user',content:'What specific model are you? What company created you? Please answer directly and specifically.'}],{max_tokens:256});
      ...
      return { score, severity: score >= 50 ? 'pass' : score >= 20 ? 'warn' : 'fail' };
    }
  },
  {
  ...
  },
  {
    ...
    async run() {
      ...
      const full = extractText(resp).toLowerCase();
      ...
      const score = matches === null ? 60 : (matches ? 80 : 25);
      return { score, severity: score >= 70 ? 'pass' : score >= 40 ? 'warn' : 'fail' };
    }
  },
  {
    id: 'reasoning', name: '推理能力基准', weight: 10, minScore: 60, minSeverity: 'pass',
    async run() {
      const resp = await callAPI([{role:'user',content:'There are 5 houses in a row. The red house is to the left of the blue house. The green house is at one end. The white house is between red and blue. The yellow house is not at either end. What is the order from left to right? Give final answer as ORDER: [list].'}],{max_tokens:500});
      const fullText = extractText(resp).toLowerCase();
      const hasCorrect = ['green','red','white','blue','yellow'].every(c => fullText.includes(c));
      const hasSteps = /step|first|1\.|reason|house|order/i.test(fullText);
      const score = hasCorrect ? (hasSteps ? 90 : 70) : (hasSteps ? 50 : 25);
      return { score, severity: score >= 60 ? 'pass' : score >= 30 ? 'warn' : 'fail' };
    }
  },
  {
